...
export default function App() {
  ...
  const [activeView, setActiveView] = useState<'login' | 'admin-dashboard' | 'admin-detail' | 'guest-survey'>('login');
    ...
    const loadInitialData = async () => {
      try {
        ...
        if (catRes.ok) {
          ...
          setCategories(catData.length > 0 ? catData : defaultCategories);
        } else {
        ...
        if (sessRes.ok) {
          ...
          setSessions(sessData.length > 0 ? sessData : defaultSessions);
        } else {
  ...
  // Persist SNS Accounts only locally
  useEffect(() => {
    ...
    if (currentKebabSessionId) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          showToast('새 일정 세션 개설 실패', 'AlertTriangle');
        }
      } catch (err) {
        showToast('서버 연결 실패', 'AlertTriangle');
      }
  ...
  const handleTriggerGuestReview = () => {
    ...
    setGuestConfirmMessage(
      `<span class="font-bold text-slate-800">${guestName}</span>님, 선택한 가능 시간대는 총 <span class="font-bold text-emerald-600">${count}개</span>입니다.<br/>이 스케줄로 최종 제출할까요?`
    );
  ...
  return (
    <div class="h-screen flex flex-col no-select text-slate-800 overflow-hidden font-sans">
      {/* Global Prototype Controller bar */}
      <div class="bg-slate-900 text-white px-4 py-2 flex flex-wrap justify-between items-center text-xs gap-2 shrink-0 shadow-md z-50">
        <div class="flex items-center gap-2">
          <span class="bg-indigo-600 px-2.5 py-0.5 rounded-full font-bold tracking-wider text-[9px]">
            PROTOTYPE CONTROLLER
          </span>
          <span class="text-slate-300 hidden md:inline">
            역할군을 전환하며 관리 시나리오와 게스트 제출 인터랙션을 체험해보세요.
          </span>
        </div>
        <div class="flex gap-1.5 overflow-x-auto custom-scrollbar pb-0.5">
          <button onClick={() => setActiveView('login')} class="demo-btn px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition">
            1. 소셜 로그인
          </button>
          <button onClick={() => setActiveView('admin-dashboard')} class="demo-btn px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition">
            2. 세션 대시보드
          </button>
          <button onClick={() => setActiveView('admin-detail')} class="demo-btn px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition">
            3. 일정 통합 히트맵
          </button>
          <button onClick={() => { setGuestName(''); setLocalGuestSchedule({}); setActiveView('guest-survey'); }} class="demo-btn px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition">
            4. 게스트 설문 수집
          </button>
        </div>
      </div>
        ...
        <header id="main-header" class="bg-white border-b border-slate-200 px-4 sm:px-6 py-2.5 flex justify-between items-center shrink-0 z-40 shadow-sm">
          ...
          <div class="flex items-center gap-3">
            <div class="text-right hidden sm:block">
              <p class="text-xs font-bold text-slate-800">이동현 중간관리자</p>
              <p class="text-[9px] text-slate-400">dh.lee@company.com</p>
            </div>
            <button
        ...
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div class="bg-white rounded-3xl p-5 w-full max-w-sm shadow-2xl">
            <div class="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setIsSettingsModalVisible(false)} class="text-slate-400 p-1.5 hover:bg-slate-50 rounded-xl transition">
                <X class="w-5 h-5" />
              </button>
            </div>
            <div class="space-y-4 text-xs font-semibold text-slate-600">
              <div>
                <label class="block font-bold text-slate-700 mb-1.5">이동현 중간관리자 연동 메일</label>
                <input type="email" value="dh.lee@company.com" readOnly class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl" />
              </div>
              <div class="space-y-2">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" defaultChecked class="rounded border-slate-300 text-indigo-600" />
              ...
              <button onClick={() => { setIsSettingsModalVisible(false); showToast('환경설정이 정상 저장되었습니다.'); }} class="w-full bg-slate-900 text-white font-bold py-3 rounded-2xl transition">
                설정 저장
              </button>
            </div>
          </div>
