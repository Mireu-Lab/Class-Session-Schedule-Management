...
import { X, Trash2, Edit3, Archive, AlertOctagon, Share2, Mail, CheckCircle, Settings, Plus, Tags, Copy, ExternalLink, QrCode } from 'lucide-react';
  ...
  return (
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white rounded-3xl p-5 w-full max-w-sm shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
        ...
        <form onSubmit={onSave} class="space-y-3">
          <div>
            <label class="block text-[10px] font-bold text-slate-600 mb-1">세션 이름</label>
            <input
              type="text"
              value={title}
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="block text-[10px] font-bold text-slate-600 mb-1">시작일</label>
              <input
                type="date"
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                required
                class="w-full text-xs p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-600 font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              />
            </div>
            <div>
              <label class="block text-[10px] font-bold text-slate-600 mb-1">종료일</label>
              <input
                type="date"
                value={endDate}
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="block text-[10px] font-bold text-slate-600 mb-1">카테고리</label>
              <select
                value={category}
            ...
            <div>
              <label class="block text-[10px] font-bold text-slate-600 mb-1">타임 슬롯 간격</label>
              <select
                value={interval}
          ...
          <div>
            <label class="block text-[10px] font-bold text-slate-600 mb-1">게스트 지정 옵션</label>
            <select
              value={guestMode}
          ...
          <div>
            <label class="block text-[10px] font-bold text-slate-600 mb-1">게스트 명단 (쉼표 구분)</label>
            <input
              type="text"
              value={guestsText}
        ...
        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200 mb-3 shrink-0">
          <div class="grid grid-cols-2 gap-2 text-[10px]">
            <input
              ...
              placeholder="카테고리 명칭"
              class="w-full p-2.5 bg-white border border-slate-200 rounded-xl font-semibold"
            />
            <select
              value={newCatColor}
      ...
      <div class="bg-white rounded-3xl p-5 w-full max-w-sm shadow-2xl">
        ...
        <div class="space-y-3">
          {snsAccounts.map((acc, idx) => (
            <div key={acc.provider} class="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-xl shadow-sm">
    ...
    <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl p-5 w-full max-w-sm shadow-2xl">
        <div class="flex justify-between items-center mb-3">
          <h3 class="text-sm font-bold text-slate-900 truncate pr-4">{title} 제어</h3>
          <button onClick={onCancel} class="text-slate-400 p-1 transition hover:bg-slate-50 rounded-lg">
            <X class="w-4 h-4" />
          </button>
        </div>
        <div class="space-y-1">
          <button
            ...
            class="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 text-xs font-bold text-slate-700 rounded-xl transition"
          >
          ...
          <button
            ...
            class="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 text-xs font-bold text-slate-700 rounded-xl transition"
          >
            <Archive class="w-4 h-4 text-slate-500" />
            <span>보관 처리 (숨김)</span>
          </button>
          <button
            onClick={() => onAction('delete')}
            class="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-red-50 text-xs font-bold text-red-600 rounded-xl transition"
          >
...
export function DeleteConfirmModal({ visible, onDeleteOption, onCancel }: DeleteConfirmModalProps) {
  ...
  return (
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl p-5 w-full max-w-sm shadow-2xl border border-slate-100 text-center">
        <div class="mx-auto w-10 h-10 bg-red-50 text-red-600 rounded-full flex items-center justify-center mb-3">
          <AlertOctagon class="w-5 h-5" />
        </div>
        <h3 class="text-base font-bold text-slate-900">세션 삭제 방법 선택</h3>
        ...
        <div class="space-y-2 mt-4">
          <button
            ...
            class="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold py-2.5 rounded-xl transition flex items-center justify-center gap-1.5 shadow-sm"
          >
          ...
          <button
            ...
            class="w-full bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-2.5 rounded-xl transition flex items-center justify-center gap-1.5 shadow-md"
          >
            <Trash2 class="w-4 h-4" />
            <span>Deep Delete (영구 물리 삭제)</span>
          </button>
          <button onClick={onCancel} class="w-full text-slate-400 hover:text-slate-600 text-xs py-1.5 font-semibold transition">
            취소
          </button>
        </div>
      </div>
    ...
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white rounded-3xl p-5 w-full max-w-sm shadow-2xl">
        ...
        <div class="space-y-3 text-xs">
          <div>
            <label class="block text-[10px] font-bold text-slate-400 mb-1">세션 관리자 할당 메일 (쉼표 구분)</label>
            <input
              type="text"
          ...
          <div>
            <label class="block text-[10px] font-bold text-slate-400 mb-1">보기 권한 공유 대상 메일 (쉼표 구분)</label>
            <input
              type="text"
...
export function ConfirmModal({ visible, message, onConfirm, onCancel }: ConfirmModalProps) {
  ...
  return (
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl p-5 w-full max-w-xs shadow-2xl text-center">
        <div class="mx-auto w-10 h-10 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mb-3">
          <CheckCircle class="w-5 h-5" />
        </div>
        <h3 class="text-sm font-bold text-slate-900">최종 일정 제출 검토</h3>
        <p class="text-[11px] text-slate-500 mt-1.5 leading-relaxed" dangerouslySetInnerHTML={{ __html: message }}></p>
        <div class="flex gap-2 pt-4">
          <button
            onClick={onCancel}
            class="flex-1 bg-slate-100 text-slate-700 font-bold py-2 rounded-xl text-xs transition hover:bg-slate-200"
          >
            수정
          </button>
          <button
            onClick={onConfirm}
            class="flex-1 bg-emerald-600 text-white font-bold py-2 rounded-xl text-xs transition hover:bg-emerald-700"
          >
            확정
          </button>
        </div>
...
export function ShareLinkModal({ visible, session, onCancel, showToast }: ShareLinkModalProps) {
