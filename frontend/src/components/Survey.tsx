            ...
            <div class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="flex-1 bg-white border-y border-slate-200 flex flex-col overflow-hidden relative">
                {filteredActiveDates.length === 0 ? (
                  ...
                  <div class="flex-1 overflow-auto custom-scrollbar relative" onTouchMove={handleTouchMove}>
                    ...
                    <div class="grid" style={{ gridTemplateColumns: `50px repeat(${filteredActiveDates.length}, minmax(70px, 1fr))` }}>
                      ...
                      {timeslots.map(time => {
                        const cols = [
                          <div key={`time-${time}`} class="time-col-sticky text-[9px] font-semibold text-slate-500 flex items-start justify-center pt-1 border-b border-slate-100">
                            {time}
                          </div>
                        ];
                        ...
                        filteredActiveDates.forEach((dateInfo) => {
                          ...
                          cols.push(
                            <div
                              ...
                              class={`guest-slot-cell border-r border-b border-slate-100 p-0.5 min-h-[44px] cursor-pointer flex items-center justify-center select-none ${
                  ...
                  {Object.keys(selectedSlotsGroupedByDay).length === 0 ? (
                    ...
                    <div class="space-y-3">
                      {Object.entries(selectedSlotsGroupedByDay).map(([dayKey, times]) => (
                        <div key={dayKey} class="bg-slate-50/50 p-3 rounded-2xl border border-slate-100">
                          <div class="text-xs font-extrabold text-indigo-700 mb-1.5">
                            {getFormattedDateFromDayKey(dayKey)}
                          </div>
                          <div class="flex flex-wrap gap-1">
                            {times.sort().map(t => (
                              <span key={t} class="bg-emerald-100 text-emerald-800 text-[10px] font-extrabold px-2 py-0.5 rounded-lg border border-emerald-200 shadow-sm">
                                {t}
                              </span>
                            ))}
                          </div>
        ...
        <section id="view-guest-survey" class="absolute inset-0 flex flex-col lg:flex-row bg-slate-50 overflow-hidden">
          ...
          <div class="w-full lg:w-80 bg-white border-b lg:border-b-0 lg:border-r border-slate-200 p-4 shrink-0 flex flex-col justify-between overflow-y-auto max-h-[45vh] lg:max-h-none z-10 shadow-sm lg:shadow-none">
            <div class="space-y-4">
              ...
              <div class="space-y-1.5 text-xs">
                <label class="block font-bold text-slate-700">참여자 성함 (본명 필수)</label>
          ...
          <div class="flex-1 p-0 sm:p-5 flex flex-col overflow-hidden relative bg-slate-50 z-0">
            ...
            <div class="flex-1 bg-white sm:rounded-xl shadow-sm border-y sm:border border-slate-200 flex flex-col overflow-hidden relative">
              <div class="flex-1 overflow-auto custom-scrollbar relative" id="guest-calendar-container" onTouchMove={handleTouchMove}>
                <div class="grid" id="guest-drag-grid" style={{ gridTemplateColumns: `50px repeat(${activeDates.length}, minmax(70px, 1fr))` }}>
                  <div class="header-corner-sticky flex items-center justify-center">
                    <Clock class="w-3.5 h-3.5 text-slate-400" />
                  </div>
                  {activeDates.map((dateInfo) => (
