...
export default function Dashboard({
  ...
  return (
    <section id="view-admin-dashboard" className="absolute inset-0 flex flex-col md:flex-row bg-slate-50 overflow-hidden">
      {/* Drawer Overlay Backdrop */}
      {isMenuOpen && (
        <div 
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-40 transition-opacity animate-in fade-in duration-200 md:hidden"
      ...
      <aside 
        className={`fixed md:static inset-y-0 left-0 w-72 bg-white border-r border-slate-200 p-5 shadow-2xl md:shadow-none z-50 md:z-20 flex flex-col justify-between transform md:transform-none transition-transform md:transition-none duration-300 ease-out ${
      ...
      >
        <div>
          <div className="flex items-center justify-between pb-5 border-b border-slate-100 mb-5">
            ...
            <button 
              onClick={() => setIsMenuOpen(false)}
              className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-500 hover:text-slate-800 transition md:hidden"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-slate-600 hover:text-slate-900 shadow-sm transition flex items-center justify-center shrink-0 md:hidden"
              title="대시보드 메뉴 열기"
            >
